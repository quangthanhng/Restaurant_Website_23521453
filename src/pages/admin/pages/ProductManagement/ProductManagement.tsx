...
export default function ProductManagement() {
  ...
  return (
    <div>
      ...
      <div className='overflow-x-auto rounded-xl border border-neutral-800 bg-neutral-950 shadow-sm hidden sm:block'>
        <table className='min-w-full divide-y divide-neutral-800'>
          <thead className='bg-neutral-900'>
            <tr>
              <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-savoria-gold'>Món ăn</th>
              <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-savoria-gold'>Danh mục</th>
              <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-savoria-gold'>Giá</th>
              <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-savoria-gold'>Trạng thái</th>
              <th className='px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-savoria-gold'>Nổi bật</th>
                ...
                <tr key={dish._id} className='hover:bg-neutral-900'>
                  ...
                  <td className='px-6 py-4'>
                    <div className='flex items-center gap-4'>
                      <img src={dish.image || 'https://via.placeholder.com/100'} alt={dish.name} className='h-12 w-12 shrink-0 rounded-lg object-cover' />
                  ...
                  <td className='whitespace-nowrap px-6 py-4'>
                    <span className='inline-flex max-w-[150px] truncate rounded-full bg-savoria-gold/20 px-3 py-1 text-xs font-medium text-savoria-gold'>{dish.categoryId.name}</span>
                  </td>
                  {/* Price */}
                  <td className='whitespace-nowrap px-6 py-4'>
                    <div>
                      <p className='font-medium text-amber-50'>{formatPrice(dish.finalPrice)}</p>
                      {dish.discount > 0 && (<p className='text-sm text-neutral-500 line-through'>{formatPrice(dish.price)}</p>)}
                    </div>
                  </td>
                  {/* Status Toggle */}
                  <td className='whitespace-nowrap px-6 py-4'>
                    <button
                      onClick={() => toggleStatusMutation.mutate({ id: dish._id, status: dish.status === 'active' ? 'inactive' : 'active' })}
                    ...
                    >
                      <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-lg transition-transform duration-200 ease-in-out ${dish.status === 'active' ? 'translate-x-6' : 'translate-x-1'}`} />
                    </button>
                  </td>
                  {/* Featured/BestSeller Toggle */}
                  <td className='whitespace-nowrap px-6 py-4'>
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${dish.bestSeller ? 'bg-amber-600/20 text-amber-400' : 'bg-neutral-800 text-neutral-400'}`}>{dish.bestSeller ? 'Best Seller' : 'Thường'}</span>
                  </td>
                  {/* Actions */}
                  <td className='whitespace-nowrap px-6 py-4'>
                    <div className='flex items-center gap-2'>
                      <button onClick={() => handleEdit(dish)} className='rounded-lg bg-savoria-gold px-3 py-1.5 text-xs font-medium text-neutral-900 transition-colors hover:bg-amber-200'>Sửa</button>
                      <button onClick={() => handleDelete(dish._id)} disabled={deleteMutation.isPending} className='rounded-lg border border-red-500 px-3 py-1.5 text-xs font-medium text-red-400 transition-colors hover:bg-red-500/20 disabled:opacity-50'>Xóa</button>
                    </div>
                  </td>
                </tr>
              ))
      ...
      <div className='block sm:hidden space-y-4'>
        ...
        {!isLoading && filteredDishes.length === 0 && (
          <div className='text-center py-8 text-neutral-400'>Không tìm thấy món ăn nào</div>
        )}
        {!isLoading && filteredDishes.map((dish) => (
          <div key={dish._id} className='flex flex-col rounded-xl border border-neutral-800 bg-neutral-950 shadow p-3 gap-2'>
            <div className='flex gap-3 items-center'>
              <img src={dish.image || 'https://via.placeholder.com/100'} alt={dish.name} className='w-16 h-16 object-cover rounded-lg border border-neutral-800 shadow' />
              <div className='flex-1 min-w-0'>
                <div className='font-bold text-savoria-gold text-base truncate'>{dish.name}</div>
                <div className='text-xs text-neutral-400 mt-1 truncate'>{dish.description}</div>
                <div className='mt-1 text-xs text-neutral-400'>Danh mục: <span className='font-semibold text-savoria-gold'>{dish.categoryId.name}</span></div>
              </div>
            </div>
            <div className='flex flex-wrap items-center gap-2 mt-1'>
              <span className='px-2 py-1 rounded-full text-xs font-bold bg-savoria-gold/20 text-savoria-gold border border-savoria-gold'>Giá: {formatPrice(dish.finalPrice)}</span>
              {dish.discount > 0 && (
                <span className='px-2 py-1 rounded-full text-xs font-bold bg-red-500/20 text-red-400 border border-red-400 line-through'>
                  {formatPrice(dish.price)}
                </span>
              )}
              <span className={`px-2 py-1 rounded-full text-xs font-bold ${dish.status === 'active' ? 'bg-green-600/20 text-green-400 border border-green-600/40' : 'bg-neutral-700/40 text-neutral-400 border border-neutral-700'}`}>{dish.status === 'active' ? 'Hoạt động' : 'Ẩn'}</span>
              <span className={`px-2 py-1 rounded-full text-xs font-bold ${dish.bestSeller ? 'bg-amber-600/20 text-amber-400 border border-amber-400' : 'bg-neutral-800 text-neutral-400 border border-neutral-700'}`}>{dish.bestSeller ? 'Best Seller' : 'Thường'}</span>
            </div>
            <div className='flex gap-2 mt-2'>
              <button className='flex-1 rounded-lg bg-savoria-gold px-2 py-1 text-xs font-semibold text-neutral-900 shadow transition-all hover:bg-amber-200' onClick={() => handleEdit(dish)}>Sửa</button>
              <button className='flex-1 rounded-lg border border-red-500 px-2 py-1 text-xs font-semibold text-red-400 shadow transition-all hover:bg-red-500/20 disabled:opacity-50' onClick={() => handleDelete(dish._id)} disabled={deleteMutation.isPending}>Xóa</button>
              <button className={`flex-1 rounded-lg px-2 py-1 text-xs font-semibold shadow transition-all ${dish.status === 'active' ? 'bg-green-600/20 text-green-400 border border-green-600/40' : 'bg-neutral-700/40 text-neutral-400 border border-neutral-700'}`} onClick={() => toggleStatusMutation.mutate({ id: dish._id, status: dish.status === 'active' ? 'inactive' : 'active' })}>
