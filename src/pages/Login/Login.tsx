  ...
  const onSubmit = handleSubmit((data) => {
    loginMutation.mutate(data, {
      onSuccess: (res) => {
        ...
        // Handle legacy API response structure (code + data) - for backward compatibility
        else {
      ...
      onError: (error) => {
        if (error instanceof AxiosError) {
          ...
          const errorMessage = errorData?.message || 'Email hoặc mật khẩu không đúng'
          const lowerMessage = errorMessage.toLowerCase()

          // Determine which field to show error under based on error message
          const isPasswordError =
            lowerMessage.includes('password') || lowerMessage.includes('mật khẩu') || lowerMessage.includes('incorrect')
          ...
          if (isPasswordError && !isEmailError) {
            // Password related error - show under password field
            setError('password', {
              type: 'server',
              message: errorMessage
            })
          } else if (isEmailError && !isPasswordError) {
            // Email related error - show under email field
            setError('email', {
              type: 'server',
              message: errorMessage
            })
          } else {
            // Generic error or both - show under both fields or email
            setError('email', {
              type: 'server',
              message: errorMessage
            })
          ...
          <div className='relative rounded-2xl border border-gray-200/50 bg-gray-50 p-8 shadow-2xl backdrop-blur-xl'>
            ...
            <div className='relative'>
              ...
              <div className='mb-8 text-center'>
                ...
                <h1 className='font-serif text-3xl font-medium text-gray-900'>Chào Mừng Trở Lại</h1>
                <p className='mt-2 text-sm text-gray-500'>Đăng nhập để tiếp tục với TS Restaurant</p>
              </div>
              ...
              <form onSubmit={onSubmit} className='space-y-5' noValidate>
                    ...
                    <input
                      ...
                      className={`w-full rounded-xl border bg-gray-100/50 py-3.5 pl-12 pr-4 text-gray-900 placeholder:text-gray-400 transition-all duration-300 focus:outline-none focus:ring-2 ${
                        errors.email
                          ...
                          : 'border-stone-200/50 focus:border-amber-500 focus:ring-savoria-gold/20 hover:border-stone-300'
                      }`}
                ...
                <div className='flex items-center justify-between'>
                  <label className='flex cursor-pointer items-center gap-2'>
                    <input
                      type='checkbox'
                      className='h-4 w-4 rounded border-stone-300 bg-gray-100 text-amber-600 focus:ring-savoria-gold/20 focus:ring-offset-0'
                    />
                    <span className='text-sm text-gray-500'>Ghi nhớ đăng nhập</span>
                  </label>
                  <Link
                    to={path.forgot_password}
                    className='text-sm text-amber-600 transition-colors hover:text-amber-300'
                  >
                    Quên mật khẩu?
                  </Link>
                </div>
                ...
                <div className='relative my-6'>
                  ...
                  <div className='relative flex justify-center'>
                    <span className='bg-gray-50 px-4 text-sm text-gray-400'>hoặc tiếp tục với</span>
                  </div>
                </div>
                ...
                <p className='mt-6 text-center text-sm text-gray-500'>
                  ...
                  <Link
                    to={path.register}
                    className='font-semibold text-amber-600 transition-colors hover:text-amber-300'
                  >
                    Đăng ký ngay
                  </Link>
                </p>
